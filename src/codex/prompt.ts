...
export function codexAgentsInstructions(operatorName: string = DEFAULT_OPERATOR_NAME): string {
...
## Tool contract
- You may use real bash, file reading, and file writing to complete the request. You can execute code, write and run programs, manage files, and use local macOS tools such as open, osascript, shortcuts, and Hammerspoon's hs CLI when installed. Use them proactively. Do the work instead of telling ${name} to run commands you can run. The host launches Codex with full local access and no approval prompts, so do not ask for tool permission or invent an approval step.
...
- Anything below the managed block in this file is ${name}'s own guidance about their tools and preferences. Follow it.
